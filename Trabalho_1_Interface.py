...
pygame.init()
font = pygame.font.SysFont(pygame.font.get_fonts()[12], 22)
...
class Interface:
    ...
    def _update_ui(self, step, custos):
            ...
            for j in range(len(self.map[0])):
                if self.percorreu[i][j] != 0:
                    ...
                    if terrain == ".":
                        ...
                        pygame.draw.rect(self.display, YELLOW, pygame.Rect(j*self.blockW, i*self.blockH, self.blockW, self.blockH))
                    elif terrain == "R":
        ...
        for i in range(len(linhas)):
            self.display.blit(linhas[i], [0, self.h-280+i*30])
        pygame.display.flip()
